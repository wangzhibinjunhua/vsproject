...
void CScanData::OnKillfocusBtData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strBTAddr, m_strBtData.GetString());
...
void CScanData::OnKillfocusBarcodeData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strBarcode, m_strBarcData.GetString()); 
...
void CScanData::OnKillfocusImei1Data() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strIMEI[0], m_strIMEI1Data.GetString());
...
void CScanData::OnKillfocusImei2Data() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strIMEI[1], m_strIMEI2Data.GetString());
...
void CScanData::OnKillfocusImei3Data() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strIMEI[2], m_strIMEI3Data.GetString());
...
void CScanData::OnKillfocusImei4Data() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strIMEI[3], m_strIMEI4Data);
...
void CScanData::OnKillfocusWifiData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strWifiAddr, m_strWifiData.GetString());
...
void CScanData::OnKillfocusEthernetMacData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strEthernetMac, m_strEthernetMacData.GetString());
...
void CScanData::OnKillfocusDrmkeyMcidData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strDrmkeyMCID, m_strDrmkeyMCIDData.GetString());
...
void CScanData::OnKillfocusSerialNoData() 
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strSerialNo, m_strSerialNoData.GetString());
...
void CScanData::OnKillfocusMeidData()
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strMeid, m_strMeidData);
...
void CScanData::OnKillfocusEsnData()
{
    ...
    strcpy_s(g_sMetaComm.sScanData.strEsn, m_strEsnData.GetString());
    ...
    if (g_sMetaComm.sWriteOption.bWriteSerialNo)
    {
        if (g_sMetaComm.SerialNumFromIMEI == true)
        {
            ...
            if (CheckSerialNoMatchWithIMEI(m_strSerialNoData) == false)
            {
                bCheckAllDataPass = false;
                char pScanMsg[256] = {0};
                sprintf_s(pScanMsg, "Serial No.[%s] not match with IMEI[%s] ->[%s]!",
                    m_strSerialNoData, m_strIMEI1Data, m_strSerinalNoByIMEI);
...
bool CScanData::CheckScanDataByIndex(int index)
{
    ...
    case Imei_4_Index:
        m_pstrScanItem = "IMEI_4";
        strcpy_s(g_sMetaComm.sScanData.strIMEI[3], m_strIMEI4Data.GetString());
        bCheckPass = CheckScanData(g_sMetaComm.sIMEIHeader_Option[3].strHeader, g_sMetaComm.sIMEIHeader_Option[3].bCheckHeader, m_strIMEI4Data, Imei);
        break;
    ...
    case Meid_Index:
        m_pstrScanItem = "Meid";
        strcpy_s(g_sMetaComm.sScanData.strMeid, m_strMeidData.GetString());
        bCheckPass = CheckScanData(g_sMetaComm.sMeidHeader_Option.strHeader, g_sMetaComm.sMeidHeader_Option.bCheckHeader, m_strMeidData, Meid);
        break;
...
bool CScanData::CheckSerialNoMatchWithIMEI(const char* pSerialNoData)
{
    ...
    return _strnicmp(m_strSerinalNoByIMEI, pSerialNoData, 15) == 0;
}
...
bool CScanData::CheckScanData(const char *pStrHeader, bool bCheckHeader, const char *pInData, E_ScanData_Type dataType)
{
    ...
    case Barcode:
        ...
        {
            sprintf_s(pScanMsg, "Barcode = \"%s\" length must be [1,%d]", pInData, BARCODE_MAX_LENGTH);
            SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
            return false;
        }
        else
        {
            ...
            {
                if (_strnicmp(pInData, g_AutoGenData.sBarcode.End, dataLength) > 0)
                {
                    sprintf_s(pScanMsg, "Barcode = \"%s\" length must be less then End_Barcode[%s]!",
                        pInData, g_AutoGenData.sBarcode.End);
                    SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                    return false;
                }
            }
    ...
    case Imei:
        if (dataLength != 15)
        {
            if (g_sMetaComm.sIMEIOption.bCheckSum && (14 == dataLength))
            {
                ...
                {
                    if ((_strnicmp(pInData, g_AutoGenData.sIMEI.End, dataLength) > 0))
                    {
                        sprintf_s( pScanMsg, "Error: IMEI[%s] should be less than End_IMEI[%s]",
                            pInData, g_AutoGenData.sIMEI.End);
                        SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                        return false;
                    }
                }
            ...
            {
                sprintf_s(pScanMsg, "%s = \"%s\",Only when enable checksum, imei length can be 14 or 15 only!",
                    m_pstrScanItem, pInData);
                SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                return false;
            }
        }
    ...
    case Wifi:
        if (dataLength != 12)
        {
            sprintf_s(pScanMsg, "%s = \"%s\" length must be 12!", m_pstrScanItem, pInData);
            SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
            return false;
        }
        ...
        if (dataType == BT)
        {
            ...
            {
                if (_strnicmp(pInData, g_AutoGenData.sBtAddress.End, dataLength) > 0)
                {
                    sprintf_s(pScanMsg, "Error: BT[%s] should be less than End_BT[%s]", pInData, g_AutoGenData.sBtAddress.End);
                    SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                    return false;
                }
            }
            ...
            if (g_sMetaComm.bCheckBTWifi)
            {
                if (_strnicmp(pInData, "9E8B", 4) == 0)
                {
                    ...
                    if (0 <= value && value <= 63)
                    {
                        sprintf_s(pScanMsg, "bt = \"%s\" illegal, use 'Identify->Engineer->Enable Check BtWifi' bypass this warning!!", pInData);
                        SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                        return false;
                    }
                }
        ...
        if (dataType == Wifi)
        {
            ...
            {
                if ((_strnicmp(pInData, g_AutoGenData.sWifiAddress.End, dataLength) > 0))
                {
                    sprintf_s(pScanMsg, "Error: Wifi[%s] should be less than End_Wifi[%s]", pInData, g_AutoGenData.sWifiAddress.End);
                    SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                    return false;
                }
            }
            ...
            if (g_sMetaComm.bCheckBTWifi)
            {
                ...
                if (_strnicmp(pInData, "000000000000", 12) == 0 || _strnicmp(pInData, "111111111111", 12) == 0 || iValue % 4 != 0)
                {
                    sprintf_s(pScanMsg, "wifi = \"%s\" illegal, use 'Identify->Engineer->Enable Check BtWifi' bypass this warning!!", pInData);
                    SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                    return false;
                }
            }
    ...
    case Ethernet_mac:
        ...
        {
            if (g_AutoGenData.bAutoGenEthernet && (_strnicmp(pInData, g_AutoGenData.sEthernetAddress.End, dataLength) > 0))
            {
                sprintf_s(pScanMsg, "Error: Ethernet[%s] should be less than End_Ethernet[%s]", pInData, g_AutoGenData.sEthernetAddress.End);
                SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                return false;
            }
            for (int i = 0; i < dataLength; i++)
    ...
    case DRMKey_MCID:
        if (dataLength != 32)
        {
            sprintf_s(pScanMsg, "%s = \"%s\" length must be 32!", m_pstrScanItem, pInData);
            SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
            return false;
        }
        else
        {
            if (g_AutoGenData.bAutoGenBarcode && (_strnicmp(pInData, g_AutoGenData.sDRMKeyMCID.End, dataLength) > 0))
            {
                sprintf_s(pScanMsg, "Error: MCID[%s] should be less than End_MCID[%s]", pInData, g_AutoGenData.sDRMKeyMCID.End);
                SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
                return false;
            }
            for (int i = 0; i < dataLength; i++)
    ...
    case Meid:
        if (dataLength != 14)
        {
            sprintf_s(pScanMsg, "%s = \"%s\" length must be 14!", m_pstrScanItem, pInData);
            SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
            return false;
        }
        else
...
void CScanData::InitScanUI()
{
    ...
    if (g_AutoGenData.bEnableAutoGen)
    {
        ...
        if (g_sMetaComm.sWriteOption.bWriteIMEI && g_AutoGenData.bAutoGenIMEI)
        {
            ...
            {
                strcpy_s(m_strBackupIMEI, g_AutoGenData.sIMEI.Next);
